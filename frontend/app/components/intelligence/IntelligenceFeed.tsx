...
import { Newspaper, Cloud, TrendingUp, FileText, Radio } from "lucide-react";
...
const IntelligenceFeed = () => {
  ...
  const safeEvents: ModelXEvent[] = Array.isArray(events) ? events : [];

  // Filter safely
  const allEvents = safeEvents;
  const newsEvents = safeEvents.filter(e => e?.domain === "social" || e?.domain === "intelligence");
  const politicalEvents = safeEvents.filter(e => e?.domain === "political");
  const weatherEvents = safeEvents.filter(e => e?.domain === "weather" || e?.domain === "meteorological");
  const economicEvents = safeEvents.filter(e => e?.domain === "economical" || e?.domain === "market");
  ...
  const renderEventCard = (item: ModelXEvent, idx: number) => {
    ...
    return (
      ...
      >
        <Card
          className={`p-4 bg-muted/30 border-l-4 hover:bg-muted/50 transition-colors ${
      ...
      <Card className="p-6 bg-card border-border">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-5 mb-4">
            ...
            <TabsTrigger value="news">NEWS ({newsEvents.length})</TabsTrigger>
            <TabsTrigger value="political">POLITICAL ({politicalEvents.length})</TabsTrigger>
            <TabsTrigger value="weather">WEATHER ({weatherEvents.length})</TabsTrigger>
            <TabsTrigger value="economic">ECONOMIC ({economicEvents.length})</TabsTrigger>
          </TabsList>
