...
from src.states.politicalAgentState import PoliticalAgentState,  ClassifiedEvent
...
class PoliticalAgentNode:
    ...
    def task_delegator_master_agent(
        ...
        print(f"   → Planned {len(tasks)} tasks for Political Worker Agents.")

        return {"generated_tasks": tasks}


    def political_worker_agent(
            self,
        state: PoliticalAgentState,
    ) -> Dict[str, Any]:
        """
        POLITICAL WORKER AGENT
    ...
    def prepare_worker_tasks(
        ...
        that will run `political_worker_agent` + `tool_node`.
        """
        tasks = state.get("generated_tasks", [])
        ...
        print(
            ...
            "parallel Political Worker Agents ---"
        )
    ...
    def feed_creator_agent(
        ...
        - Builds a consolidated political feed and stores it in:
        ...
        bulletin = f"""🇱🇰 SRI LANKA POLITICAL FEED
